...
userid nvarchar2(20) not null  --외래키
);

--외래키 참조 및 연속삭제 설정 (멤버 테이블참조)

alter table applybook
add constraint fk_applybook foreign key(userid)
...
add constraint fk_booklist foreign key(userid)
REFERENCES library_member(userid)
on delete cascade;
...
alter table library_board
add constraint fk_library_board foreign key(writer)
REFERENCES library_member(userid)
on delete cascade;
...
add  constraint fk_book_report_writer foreign key (writer)
REFERENCES library_member(userid) 
on delete cascade;
